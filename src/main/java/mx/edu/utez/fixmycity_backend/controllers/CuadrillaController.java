...
public class CuadrillaController {
    ...
    public ResponseEntity<ApiResponse> listarCuadrillas(@RequestParam(defaultValue = "todas") String estado) {
    ...
    @GetMapping("/api/admin/squads/{id}/volunteers/edit")
    public ResponseEntity<ApiResponse> voluntariosParaEdicion(@PathVariable int id, @RequestParam int idMunicipio) {
        return ResponseEntity.ok(cuadrillaService.listarVoluntariosParaEdicion(idMunicipio, id));
    }

    @PutMapping("/api/admin/squads/{id}/members")
    public ResponseEntity<ApiResponse> editarMiembros(@PathVariable int id, @RequestBody @Valid CuadrillaRequest request) {
        ApiResponse response = cuadrillaService.editarMiembrosCuadrilla(id, request);
